...
urlpatterns = [
    ...
    url(r'^start/(?P<unit>\w+)/(?P<group>\w+)/$', views.group_page, name='group_page'),
    url(r'^articles/(?P<theme>\w+)/$', views.articles_page, name='articles_page'),
    url(r'^start/(?P<unit>\w+)/(?P<group>\w+)/article/(?P<id>\w+)/$', views.section_article_page, name='section_article_page'),
